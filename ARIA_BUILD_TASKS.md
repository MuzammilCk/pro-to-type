...
Two-part fix:
1. **Fetch the models.** Three files are needed: `models/yolov5s.onnx` (the general-purpose detector in `detector.py`), plus `face_detection_yunet_2026may.onnx` and `face_recognition_sface_2021dec.onnx` (the face pipeline in `face_engine.py`) from the OpenCV Zoo (`github.com/opencv/opencv_zoo`, under `models/face_detection_yunet/` and `models/face_recognition_sface/`). Since `models/` is correctly gitignored, add a small setup script (or a README section) that fetches these automatically — this isn't just about your own machine: the competition's final submission requires build instructions a judge can follow from a fresh clone, and right now a fresh clone can't run at all.
2. **Fix the model filename.** Update `FACE_DET_MODEL` in `face_engine.py` from `face_detection_yunet_2023mar.onnx` to `face_detection_yunet_2026may.onnx` — the old name is a fixed-input-shape model meant for OpenCV 4.x's DNN engine; OpenCV 5.x's ONNX Runtime engine needs the dynamic-shape 2026may variant, which is the Zoo's own current default. Confirm with `pip show opencv-python` that 5.x is actually what's installed in this environment, since `requirements.txt` only sets a floor (`>=5.0.0`) rather than pinning it.
